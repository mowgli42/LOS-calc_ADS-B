...
class OpenSkyClient:
    ...
    def parse_state_vector(self, state_vector: List) -> Optional[Dict]:
        ...
        try:
            ...
            origin_country = state_vector[2] if len(state_vector) > 2 and state_vector[2] else None
            ...
            # For military aircraft, we'll set a special carrier_code based on the military group
            # This will be set later during filtering, but we need to preserve the original carrier_code
            # for commercial aircraft
            
            return {
                'icao24': icao24,
                'callsign': callsign,
                'carrier_code': carrier_code,  # Commercial carrier code or None
    ...
    def is_icao24_in_range(self, icao24: str, address_ranges: List[tuple]) -> bool:
        """
        Check if ICAO24 address falls within any of the specified address ranges.
        
        Args:
            icao24: ICAO24 address as hex string (e.g., 'AE1234')
            address_ranges: List of (min, max) tuples with hex strings
        
        Returns:
            True if address is within any range
        """
        try:
            # Convert ICAO24 to integer for comparison
            icao_int = int(icao24, 16)
            
            for min_hex, max_hex in address_ranges:
                min_int = int(min_hex, 16)
                max_int = int(max_hex, 16)
                if min_int <= icao_int <= max_int:
                    return True
            return False
        except (ValueError, TypeError):
            return False
    
    def filter_by_military_groups(self, states: List[List], military_group_ids: List[str]) -> List[Dict]:
        """
        Filter aircraft states by military group IDs.
        
        Args:
            states: List of OpenSky state vectors
            military_group_ids: List of military group IDs (e.g., ['US_MIL', 'UK_MIL'])
        
        Returns:
            List of parsed aircraft dictionaries matching the military groups
        """
        from config import MILITARY_GROUPS
        ...
        for state_vector in states:
            ...
            # Check each requested military group
            for group_id in military_group_ids:
                group = MILITARY_GROUPS.get(group_id)
                if not group:
                    continue
                ...
                # Check address range
                if group.get('address_ranges'):
                    if self.is_icao24_in_range(icao24, group['address_ranges']):
                        matched = True
                
                # Check origin country (as additional filter or fallback)
                if not matched and group.get('origin_countries') and origin_country:
                    if origin_country in group['origin_countries']:
                        matched = True
                
                if matched:
                    # Set carrier_code to military group ID for compatibility with communication analyzer
                    aircraft['carrier_code'] = group_id
                    filtered.append(aircraft)
                    break
        
        logger.info(f"Filtered to {len(filtered)} military aircraft from {len(states)} total")
        return filtered
