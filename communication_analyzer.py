...
def calculate_connectivity_metrics_at_range(aircraft_list: List[Dict],
                                            distances: List[Dict],
                                            test_range_km: float) -> Dict:
    """
    Calculate connectivity metrics at a specific communication range.
    Used for range sensitivity analysis.
    
    Args:
        aircraft_list: List of aircraft dictionaries
        distances: List of distance calculations
        test_range_km: Communication range to test (km)
    
    Returns:
        Dictionary with connectivity metrics:
        {
            'direct_connections': int,
            'total_reachable_pairs': int,  # direct + 1-hop + 2-hop + 3-hop
            'graph_density': float,  # edges / max_possible_edges
            'average_degree': float
        }
    """
    # Create a uniform carrier_ranges dict with the test range
    test_carrier_ranges = {}
    for aircraft in aircraft_list:
        carrier = aircraft.get('carrier_code')
        if carrier:
            test_carrier_ranges[carrier] = test_range_km
    
    # Build graph with test range
    graph = build_connectivity_graph(aircraft_list, distances, test_carrier_ranges)
    
    # Calculate direct connections (edges)
    total_edges = sum(len(neighbors) for neighbors in graph.values()) // 2  # Divide by 2 for undirected
    
    # Calculate total reachable pairs (direct + multi-hop)
    all_aircraft = {a['icao24'] for a in aircraft_list}
    reachable_pairs = set()
    ...
    for aircraft in aircraft_list:
        ...
        # Multi-hop reachable (up to 3 hops)
        hop1_reachable = bfs_with_depth_limit(graph, icao, max_depth=2)
        hop2_reachable = bfs_with_depth_limit(graph, icao, max_depth=3)
        hop3_reachable = bfs_with_depth_limit(graph, icao, max_depth=4)
    ...
    return {
        'direct_connections': total_edges,
        'total_reachable_pairs': len(reachable_pairs),
